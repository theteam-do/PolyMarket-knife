...
impl AttackExecutor {
    ...
    async fn attack_low_gas(&self, target: &TargetMarket) -> Result<()> {
        ...
        // 模拟攻击准备时间
        let prep_time =
            std::time::Duration::from_millis(50 + (self.config.strategy.attack_gas_limit % 100));
        tokio::time::sleep(prep_time).await;

        tracing::debug!(
            "Low-gas attack simulation completed for market {}",
            target.market
        );
        Ok(())
    }
    ...
    async fn attack_nonce_gap(&self, target: &TargetMarket) -> Result<()> {
        ...
        tracing::debug!(
            "Nonce-gap attack simulation completed for market {}",
            target.market
        );
        Ok(())
    }
    ...
    async fn attack_insufficient_balance(&self, target: &TargetMarket) -> Result<()> {
        ...
        tracing::debug!(
            "Simulated balance: ${:.2} (80% of target liquidity)",
            simulated_balance
        );

        // 模拟攻击检测时间
        let detection_time =
            std::time::Duration::from_millis(50 + (simulated_balance as u64 % 100));
        tokio::time::sleep(detection_time).await;

        tracing::debug!(
            "Insufficient-balance attack simulation completed for market {}",
            target.market
        );
        Ok(())
    }
