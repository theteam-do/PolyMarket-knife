...
mod tests {
    ...
    fn test_level_creation() {
        let level = Level { price: 0.50, size: 100.0 };
    ...
    fn test_mid_price() {
        ...
        book.bids = vec![Level { price: 0.50, size: 100.0 }];
        book.asks = vec![Level { price: 0.52, size: 100.0 }];
        book.update_best();
    ...
    fn test_spread_bps() {
        let mut book = OrderBook::new("test".to_string());
        book.bids = vec![Level { price: 0.50, size: 100.0 }];
        book.asks = vec![Level { price: 0.51, size: 100.0 }];
        book.update_best();
