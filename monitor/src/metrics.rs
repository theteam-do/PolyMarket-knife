...
impl Metrics {
    pub fn new() -> Self {
        ...
        let orders_cancelled =
            Counter::new("orders_cancelled_total", "Total orders cancelled").unwrap();
        let orders_failed = Counter::new("orders_failed_total", "Total orders failed").unwrap();
        let opportunities_found =
            Counter::new("opportunities_found_total", "Total opportunities found").unwrap();
        ...
        let signals_generated =
            Counter::new("signals_generated_total", "Total signals generated").unwrap();
        let total_position = Gauge::new("total_position_usd", "Total position in USD").unwrap();
        let position_per_market =
            Gauge::new("position_per_market_usd", "Position per market in USD").unwrap();
        let api_latency = Histogram::with_opts(
            prometheus::HistogramOpts::new("api_latency_ms", "API latency in ms").buckets(vec![
        ...
        let consecutive_losses =
            Gauge::new("consecutive_losses", "Consecutive losses count").unwrap();
