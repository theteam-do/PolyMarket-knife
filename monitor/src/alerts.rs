...
#[derive(Debug, Clone)]
pub enum AlertType {
    DailyLossWarning { current: Decimal, threshold: Decimal },
    DailyLossExceeded { current: Decimal, threshold: Decimal },
    PositionExceeded { current: Decimal, threshold: Decimal },
    HighErrorRate { rate: f64, threshold: f64 },
    HighLatency { latency_ms: f64, threshold_ms: f64 },
...
impl AlertManager {
    ...
    pub fn check_daily_loss(&mut self, current_pnl: Decimal) -> Vec<Alert> {
        ...
        let warning_threshold = threshold * Decimal::from_f64_retain(self.config.daily_loss_warning_pct).unwrap();
        
        if current_pnl < -threshold {
            let msg = format!("日亏损超限！当前：${}, 阈值：${}", current_pnl, threshold);
            error!("CRITICAL: {}", msg);
            alerts.push(Alert {
                alert_type: AlertType::DailyLossExceeded { current: current_pnl, threshold },
                severity: Severity::Critical,
                message: msg,
                timestamp: Instant::now(),
            });
        } else if current_pnl < -warning_threshold {
            let msg = format!("日亏损接近阈值！当前：${}, 警告线：${}", current_pnl, warning_threshold);
            warn!("WARNING: {}", msg);
            alerts.push(Alert {
                alert_type: AlertType::DailyLossWarning { current: current_pnl, threshold: warning_threshold },
    ...
    pub fn check_position(&mut self, current_position: Decimal) -> Vec<Alert> {
        ...
        if current_position > self.config.position_threshold {
            let msg = format!("持仓超限！当前：${}, 阈值：${}", current_position, self.config.position_threshold);
            error!("CRITICAL: {}", msg);
            alerts.push(Alert {
                alert_type: AlertType::PositionExceeded { current: current_position, threshold: self.config.position_threshold },
                severity: Severity::Critical,
                message: msg,
                timestamp: Instant::now(),
            });
    ...
    pub fn check_latency(&mut self, latency_ms: f64) -> Vec<Alert> {
        ...
        if latency_ms > self.config.latency_threshold_ms {
            let msg = format!("延迟过高！当前：{}ms, 阈值：{}ms", latency_ms, self.config.latency_threshold_ms);
            warn!("WARNING: {}", msg);
            alerts.push(Alert {
                alert_type: AlertType::HighLatency { latency_ms, threshold_ms: self.config.latency_threshold_ms },
                severity: Severity::Warning,
                message: msg,
                timestamp: Instant::now(),
            });
    ...
    pub fn check_consecutive_losses(&mut self, count: u32) -> Vec<Alert> {
        ...
        if count >= self.config.consecutive_loss_threshold {
            let msg = format!("连续亏损过多！当前：{}次，阈值：{}次", count, self.config.consecutive_loss_threshold);
            error!("CRITICAL: {}", msg);
            alerts.push(Alert {
                alert_type: AlertType::ConsecutiveLosses { count, threshold: self.config.consecutive_loss_threshold },
                severity: Severity::Critical,
                message: msg,
                timestamp: Instant::now(),
            });
    ...
    pub fn record_order_failure(&mut self, order_id: &str, reason: &str) -> Vec<Alert> {
        ...
        warn!("WARNING: {}", msg);
        let alert = Alert {
            alert_type: AlertType::OrderFailed { order_id: order_id.to_string(), reason: reason.to_string() },
            severity: Severity::Warning,
            message: msg,
            timestamp: Instant::now(),
        };
